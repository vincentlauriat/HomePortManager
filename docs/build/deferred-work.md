...
- source_spec: `docs/build/spec-1-1-fenêtre-centre-de-contrôle-et-tableau-de-bord-global.md`
  summary: Le nom de machine du MachineBanner est rogné horizontalement à mi-hauteur — seul le token windowTitle est touché.
  evidence: Observé à l'écran sur raspcorse et raspyellow. Le second Text du même VStack (l'hôte, en Theme.data / mono 12) est intact, donc le défaut est propre à Theme.windowTitle = sans(22, .light), tracking -0.33, lineSpacing 3.3. Il reste de l'espace libre dans le bandeau au-dessus du texte rogné, donc ce n'est pas un manque de place. Hypothèses éliminées par test ou lecture : (1) NSHostingView sans safe area — corrigé via NSHostingController, sans effet sur ce point ; (2) bande de window-toolbar du NavigationSplitView — masquée via .toolbar(.hidden, for: .windowToolbar), sans effet ; (3) apparence sombre héritée — corrigée via NSAppearance(.aqua), ce qui a supprimé la bande grise mais pas le rognage ; (4) frame trop court — .fixedSize(horizontal: false, vertical: true) posé, sans effet ; (5) clip explicite ou hauteur fixe — aucun .clipped(), aucun .frame(height:) et aucun lineLimit sur MachineBanner ; (6) fonte custom introuvable — la pile sans résout à nil sur cette machine, donc .system(size: 22, weight: .light), pas de Font.custom en cause. Piste non testée : l'asymétrie styled/themeFont (styled pose la fonte et le tracking mais pas le lineSpacing du token) ; à instrumenter dans une SwiftUI Preview plutôt que par relance de l'app.
