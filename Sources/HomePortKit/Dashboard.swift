...
/// Host is what follows the last `@` (a bare `ssh` is already a host).
///
/// Pure and total: an empty target, a dangling `user@`, or a host outside the DNS
...
/// healthz check probes. Scheme is plain http on the tailnet, path is the root.
///
/// Pure and total: a port outside 1…65535 (the `URLComponents.port` setter raises on a
/// negative one) yields `nil`, never a crash.
